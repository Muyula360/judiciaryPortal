...
export default function Header({ 
  ...
  return (
    <header className={`sticky top-0 z-20 backdrop-blur-xl border-b transition-colors duration-300 ${isDarkTheme ? 'bg-slate-950/80 border-slate-800' : 'bg-red-500 border-red-500'}`}>
      <div className="px-4 sm:px-6 lg:px-28 py-1.5">
        ...
        <div className="flex-1 flex-wrap justify-center items-center gap-4 text-sm text-white">
            <span className="inline-flex items-center gap-2 me-5">
              <Fa.FaPhone className="w-4 h-4 text-yellow-500" />
              Toll-Free: <strong>0800 750 247</strong>
            </span>
            <span className="inline-flex items-center gap-2 me-5">
              <Fa.FaEnvelope className="w-4 h-4 text-yellow-500" />
              Email: <strong>info@judiciary.go.tz</strong>
            </span>
            <span className="inline-flex items-center gap-2">
              <Fa.FaWhatsapp className="w-4 h-4 text-yellow-500" />
              WhatsApp: <strong>0752 500 400</strong>
            </span>
          </div>
          <div className="flex items-center gap-2">
            <a className="text-white font-semibold me-5"  onClick={logout} href='#'>Logout</a>
            <button
              onClick={toggleTheme}
