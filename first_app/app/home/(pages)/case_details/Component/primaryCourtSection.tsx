...
import caseDataJson from '../primary-case-data.json';


const PrimaryCourtSection = () => {
...
type CaseRecord = {
  ...
  // Search state for Primary Court tab
  ...
  const primaryCourtCases = allCases.filter(c => c.court.includes('Primary Court'));
  
  // Get unique values for Primary Court search filters
  const searchCaseTypes = Array.from(new Set(primaryCourtCases.map(c => c.case_type_name))).filter(Boolean);
  const searchCourtNames = Array.from(new Set(primaryCourtCases.map(c => c.court))).filter(Boolean);
  const searchCaseYears = Array.from(new Set(primaryCourtCases.map(c => c.case_year))).filter(Boolean).sort();
  
  // Handle search from Primary Court tab
  const handleSearch = () => {
    ...
    setTimeout(() => {
      const caseMatch = primaryCourtCases.find(c => 
        c.case_number.toLowerCase().includes(searchCaseNumber.toLowerCase()) &&
        c.case_year === searchCaseYear &&
        c.case_type_name === searchCaseType &&
            ...
            <div className="space-y-6">
              <div className={`rounded-xl border ${isDarkTheme ? 'border-slate-500' : 'border-slate-50 bg-slate-600/10'}`}>
                <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between p-6">
                  <div>
                    <p className="text-md uppercase font-semibold text-red-500">Primary Court case</p>
                    <h3 className={`mt-2 text-2xl font-semibold ${isDarkTheme ? 'text-white' : 'text-slate-900'}`}>
                      {searchResult.case_number}/{searchResult.case_year}
                    </h3>
                    <p className={`mt-2 max-w-xl text-sm leading-6 ${isDarkTheme ? 'text-slate-400' : 'text-slate-600'}`}>
                      {searchResult.court} · {searchResult.case_type_name}
                    </p>
                  </div>

                  <div className="flex flex-wrap items-center gap-3">
                    <span className={`inline-flex items-center rounded-xl px-3 py-1 text-xs font-semibold ${
                      searchResult.status === 'Decided' ? 'bg-emerald-100 text-emerald-800' :
                      searchResult.status === 'Pending' ? 'bg-amber-100 text-red-500' :
                      'bg-sky-100 text-sky-800'
                    }`}>
                      {searchResult.status}
                    </span>
                    <span className="rounded-xl bg-slate-200/90 px-3 py-1 text-xs font-medium text-slate-700">
                      {searchResult.zone}
                    </span>
                    <span className="rounded-xl bg-slate-200/90 px-3 py-1 text-xs font-medium text-slate-700">
                ...
                <div className={`border-t  px-6 py-4 ${isDarkTheme ? 'border-slate-200' : 'border-slate-900'}`}>
                  <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
                    ...
                    <div className="rounded-xl bg-white/10 p-4">
                      <p className="text-md uppercase font-semibold text-red-500">Magistrate</p>
                      <p className={`mt-2 ${isDarkTheme ? 'text-slate-200' : 'text-slate-700'}`}>{searchResult.magistrate}</p>
                    </div>
                    <div className="rounded-xl bg-white/10 p-4">
                    ...
                    <div className="rounded-xl bg-white/10 p-4">
                      <p className="text-md uppercase font-semibold text-red-500">Decided Date</p>
                      <p className={`mt-2 ${isDarkTheme ? 'text-slate-200' : 'text-slate-700'}`}>{searchResult.decided_date ?? 'N/A'}</p>
                    </div>
                  </div>
                </div>
                  ...
                  <div className="grid gap-4 lg:grid-cols-2">
                    <div className="rounded-xl  bg-white/10 p-5">
                      ...
                      <div className="mt-3 space-y-1 text-sm leading-6 ">
                        ...
                        <p className={`${isDarkTheme ? 'text-white' : 'text-slate-700'}`}><span className={`font-semibold`}>Type:</span> {searchResult.case_type_name}</p>
                        <p className={`${isDarkTheme ? 'text-white' : 'text-slate-700'}`}><span className={`font-semibold`}>Status:</span> {searchResult.status}</p>
                      </div>
                    </div>
                    <div className="rounded-xl p-5 bg-white/10">
                      <p className="text-md uppercase font-semibold text-red-500">Wadaawa details</p>
                      <div className="mt-3 space-y-3">
                        <p className={`text-lg font-semibold ${isDarkTheme ? 'text-white' : 'text-slate-700'}`}>{searchResult.concat}</p>
                      </div>
                    </div>
                  </div>
                </div>
...
export default PrimaryCourtSection
